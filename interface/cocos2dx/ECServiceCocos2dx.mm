...
void ECServiceCocos2dx::showElva(string playerName,string playerUid,int serverId,string playerParseId,string playershowConversationFlag,cocos2d::ValueMap& config) {
    ...
    cocos2d::ValueMap otherButtonsMap = config["hs-custom-metadata"].asValueMap();
    cocos2d::ValueVector tagValue;
    
    NSMutableString * NtagValue  = [[NSMutableString alloc]init];
    
    tagValue = otherButtonsMap["hs-tags"].asValueVector();
    for(int i = 0; i <tagValue.size();i++ ){
        std::string tag =  tagValue[i].asString();
        NSString * test = elvaParseCString(tag.c_str());
        [NtagValue appendString: test];
        if(i == tagValue.size() -1){
            break;
        }
        [NtagValue appendString: @","];
    }
    
    std::string versionCode = otherButtonsMap["VersionCode"].asString();
    NSString* NversionCode  = elvaParseCString(versionCode.c_str());

    NSMutableDictionary *map1 = [[NSMutableDictionary  alloc]initWithCapacity:0];
    [map1 setObject:NtagValue forKey:@"hs-tags"];
    [map1 setObject:NversionCode forKey:@"VersionCode"];
    NSMutableDictionary *map = [[NSMutableDictionary  alloc]initWithCapacity:0];
    [map setObject:map1 forKey:@"hs-custom-metadata"];
...
void ECServiceCocos2dx::showSingleFAQ(string faqId,cocos2d::ValueMap& config) {
    ...
    tagValue =  otherButtonsMap["hs-tags"].asValueVector();
    for(int i = 0; i <tagValue.size();i++ ){
          std::string tag =  tagValue.at(i).asString();
        NSString * test = elvaParseCString(tag.c_str());
        [NtagValue appendString: test];
        if(i == tagValue.size() -1){
            break;
...
void ECServiceCocos2dx::showConversation(string playerUid,int serverId,cocos2d::ValueMap& config){
    //解析config转换为NSMutableDictionary
    cocos2d::ValueMap otherButtonsMap = config["hs-custom-metadata"].asValueMap();
    cocos2d::ValueVector tagValue;
    NSMutableString * NtagValue  = [[NSMutableString alloc]init];
